...
/**
 * A panel that displays a two-dimensional animation that is constructed using a
 * scene graph to implement hierarchical modeling. There is a checkbox that
...
public class SceneGraph extends JPanel {
	...
	private TransformedObject Wiel1;
	private TransformedObject Wiel2;
	private TransformedObject Wiel3;
	private TransformedObject Wiel4;
	private TransformedObject Wiel5;
	private TransformedObject Wiel6;
	...
	private void createWorld() {
		...
		triangle1.setScale(1, 2.5).setTranslation(0, -3).setColor(Color.BLUE);

		triangle2 = new TransformedObject(filledTriangle);
		triangle2.setScale(0.85, 1.875).setTranslation(-2, -0.5).setColor(Color.PINK);

		triangle3 = new TransformedObject(filledTriangle);
		triangle3.setScale(0.5, 1.25).setTranslation(2, 0.5).setColor(Color.GREEN);
		...
		rectangle1.setScale(3, 0.25).setTranslation(0, -0.5).setRotation(-22.5).setColor(Color.RED);

		rectangle2 = new TransformedObject(filledRect);
		rectangle2.setScale(2.25, 0.1875).setTranslation(-2, 1.4).setRotation(-22.5).setColor(Color.RED);

		rectangle3 = new TransformedObject(filledRect);
		rectangle3.setScale(1.5, 0.125).setTranslation(2, 1.75).setRotation(-22.5).setColor(Color.RED);
		
		world.add(rectangle1);
		world.add(rectangle2);
		world.add(rectangle3);
		
		Wiel1 = new TransformedObject(filledPolygon);
		Wiel1.setScale(0.8, 0.8).setTranslation(-1.25, 0).setColor(Color.BLACK);
		Wiel2 = new TransformedObject(filledPolygon);
		Wiel2.setScale(0.8, 0.8).setTranslation(1.25, -1).setColor(Color.BLACK);

		Wiel3 = new TransformedObject(filledPolygon);
		Wiel3.setScale(0.6, 0.6).setTranslation(-3, 1.8).setColor(Color.BLACK);
		Wiel4 = new TransformedObject(filledPolygon);
		Wiel4.setScale(0.6, 0.6).setTranslation(-1, 1).setColor(Color.BLACK);

		Wiel5 = new TransformedObject(filledPolygon);
		Wiel5.setScale(0.4, 0.4).setTranslation(1.3, 2.05).setColor(Color.BLACK);
		Wiel6 = new TransformedObject(filledPolygon);
		Wiel6.setScale(0.4, 0.4).setTranslation(2.75, 1.45).setColor(Color.BLACK);
	...
	public void updateFrame() {
		...
		Wiel1.setRotation(frameNumber * 1);
		Wiel2.setRotation(frameNumber * 1);
		Wiel3.setRotation(frameNumber * 1);
		Wiel4.setRotation(frameNumber * 1);
		Wiel5.setRotation(frameNumber * 1);
		Wiel6.setRotation(frameNumber * 1);
	}
	...
	/**
	 * Defines a subclass, CompoundObject, of SceneGraphNode to represent an object
	 * that is made up of sub-objects. Initially, there are no sub-objects. Objects
	 * are added with the add() method.
	 */
	...
	/**
	 * TransformedObject is a subclass of SceneGraphNode that represents an object
	 * along with a modeling transformation to be applied to that object. The object
	 * must be specified in the constructor. The transformation is specified by
	 * calling the setScale(), setRotate() and setTranslate() methods. Note that
	 * each of these methods returns a reference to the TransformedObject as its
	 * return value, to allow for chaining of method calls. The modeling
	 * transformations are always applied to the object in the order scale, then
	...
	private static SceneGraphNode filledPolygon = new SceneGraphNode() {
		void doDraw(Graphics2D g) { // width = 1, height = 1, center of base is at (0,0);
			...
			for (int i = 1; i <= 6; i++) {
				if (i == 1) {
					path.moveTo((0.5 * Math.cos((2 * Math.PI / 6) * i)), (0.5 * Math.sin((2 * Math.PI / 6) * i)));
				} else {
					path.lineTo((0.5 * Math.cos((2 * Math.PI / 6) * i)), (0.5 * Math.sin((2 * Math.PI / 6) * i)));
				}
			}
